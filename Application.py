...
def Api_connect():
    ...
    api_key = "AIzaSyCmtbzWpABA38OeoSuAlg23YfOGrEH9se0"
...
if selected == "SQL Migration":
    ...
    chann_name = []         
    for data in col.find({},{'_id':0}):
        chann_name.append(data["channel_details"][0]["channel_name"])  # This part slice the channel names alone and store them in list.
    ...
    if sql_button:
        if channel_selected in channel_options:
            with st.spinner("Migrating Data...:running:"):
                t.sleep(10)
            def get_channel():                     # This part receive the channel name from user and fetch the channel details from MongoDB
